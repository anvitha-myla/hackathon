import type { TwinFrame } from '@/store/appStore'

function wsUrl(): string {
  const proto = window.location.protocol === 'https:' ? 'wss' : 'ws'
  return `${proto}://${window.location.host}/ws/v1/twin/stream`
}

export type StreamHandlers = {
  onFrame: (frame: TwinFrame) => void
  onHello?: (msg: Record<string, unknown>) => void
  onState?: (state: 'connecting' | 'live' | 'offline') => void
  onLog?: (level: 'info' | 'warn' | 'err' | 'ok', message: string) => void
  onStartAck?: () => void
  onPauseAck?: () => void
  onResetAck?: () => void
}

/** WebSocket client — never auto-starts simulation (UI owns lifecycle). */
export class TwinStreamClient {
  private ws: WebSocket | null = null
  private handlers: StreamHandlers
  private shouldReconnect = true
  private speedX: 1 | 5 | 10 = 1

  constructor(handlers: StreamHandlers) {
    this.handlers = handlers
  }

  connect() {
    this.handlers.onState?.('connecting')
    this.ws = new WebSocket(wsUrl())
    this.ws.onopen = () => {
      this.handlers.onState?.('live')
      this.handlers.onLog?.('ok', 'WebSocket connected (idle — press Start to run)')
      // Configure only; do not start
      this.send({
        action: 'configure',
        hz: 10,
        dt_s: 0.1,
        speed_x: this.speedX,
        include_ai_advisory: false,
      })
    }
    this.ws.onclose = () => {
      this.handlers.onState?.('offline')
      this.handlers.onLog?.('warn', 'WebSocket closed — reconnecting in 1.2s')
      if (this.shouldReconnect) setTimeout(() => this.connect(), 1200)
    }
    this.ws.onerror = () => this.handlers.onLog?.('err', 'WebSocket error')
    this.ws.onmessage = (ev) => {
      const msg = JSON.parse(ev.data)
      if (msg.type === 'hello') this.handlers.onHello?.(msg)
      if (msg.type === 'frame') this.handlers.onFrame(msg.payload as TwinFrame)
      if (msg.type === 'start_ack') this.handlers.onStartAck?.()
      if (msg.type === 'pause_ack') this.handlers.onPauseAck?.()
      if (msg.type === 'reset_ack') this.handlers.onResetAck?.()
      if (msg.type === 'error') this.handlers.onLog?.('err', String(msg.detail))
    }
  }

  setSpeed(speed: 1 | 5 | 10) {
    this.speedX = speed
  }

  send(payload: Record<string, unknown>) {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      this.ws.send(JSON.stringify(payload))
    }
  }

  start(speed: 1 | 5 | 10 = this.speedX) {
    this.speedX = speed
    this.send({
      action: 'start',
      hz: 10,
      dt_s: 0.1,
      speed_x: speed,
      include_ai_advisory: false,
    })
  }

  pause() {
    this.send({ action: 'pause' })
  }

  resetStream() {
    this.send({ action: 'reset' })
  }

  disconnect() {
    this.shouldReconnect = false
    this.ws?.close()
  }
}

export async function postSetpoints(body: Record<string, number>) {
  await fetch('/api/v1/twin/setpoints', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  })
}

export async function postReset() {
  const r = await fetch('/api/v1/twin/reset', { method: 'POST' })
  return r.json()
}

export async function fetchAdvisory() {
  const r = await fetch('/api/v1/twin/advisory', { method: 'POST' })
  return r.json()
}

export async function fetchPhysicsMode() {
  const r = await fetch('/api/v1/twin/physics-mode')
  return r.json()
}

export async function postTimeJump(tMin: number) {
  const r = await fetch('/api/v1/twin/jump', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ t_min: tMin, keyframe_every_s: 10 }),
  })
  if (!r.ok) throw new Error(await r.text())
  return r.json()
}
